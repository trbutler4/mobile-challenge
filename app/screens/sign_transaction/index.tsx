...
export default function SignTransactionScreen() {
  ...
  function handleSignTransaction() {
    ...
    if (result) {
      router.replace({
        pathname: "/screens/transaction_signature/[param]",
        params: { signature: result.txDER, rlp: result.raw },
      });
    } else {
  ...
  return (
    <View className="flex-1 justify-between items-center m-10">
      ...
      <View>
        ...
        <Text>
          <Text className="font-bold">Gas Limit: </Text>
          {transaction.gasLimit}
        </Text>
        <Text>
          <Text className="font-bold">Max Gas Fee: </Text>
          {transaction.maxFeePerGas}
        </Text>
        <Text>
          <Text className="font-bold">Max Priority Gas Fee: </Text>
          {transaction.maxPriorityFeePerGas}
        </Text>
        <Text>
          <Text className="font-bold">Nonce: </Text>
          {transaction.nonce}
        </Text>
        <Text>
